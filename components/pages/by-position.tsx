...
export function EmployeeByPosition({ stats }: EmployeeByPositionProps) {
  ...
  const renderBarLabel = (props: any) => {
    ...
    const totalEmployees = positionData.reduce((sum, item) => sum + item.male + item.female, 0)
  ...
  const renderPieLabel = (entry: any) => {
    const total = positionCategory.reduce((sum, item) => sum + item.value, 0)
    const percent = total ? ((entry.value / total) * 100).toFixed(1) : "0.0"
    return `${percent}%`
  }

  const getPositionValue = (label: string) =>
    positionCategory.find((item) => item.name.toLowerCase() === label.toLowerCase())?.value ?? 0
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ...
          { label: "JFT", value: getPositionValue("JFT"), color: "from-cyan-500 to-cyan-600" },
          { label: "JFU", value: getPositionValue("JFU"), color: "from-purple-500 to-purple-600" },
        ].map((item) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <th className="px-4 py-3 text-left font-semibold text-slate-700">Jabatan</th>
              ...
              <tbody>
                {positionData.map((row) => (
                  <tr key={row.position} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-slate-700">{row.position}</td>
                    ...
                    <td className="px-4 py-3 text-center font-semibold text-blue-600">{row.male + row.female}</td>
                  </tr>
                ))}
              </tbody>
            </table>
