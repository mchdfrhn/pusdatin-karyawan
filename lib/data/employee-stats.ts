...
const CATEGORY_PALETTE = ["#3b82f6", "#8b5cf6", "#ec4899", "#f59e0b", "#10b981", "#06b6d4", "#14b8a6", "#f97316"]
...
const STATUS_ALIASES: Record<StatusKey, string[]> = {
  ...
  PPnPN: ["ppnpn", "ppn pn", "pegawai pemerintah non pns", "pegawai pemerintah non pegawai negara"],
  KI: ["ki", "karyawan insidental", "karyawan insidentil", "karyawan incidental"],
}
...
const FIELD_ALIASES = {
  ...
  age: ["age", "umur", "usia", "age_years", "ageyears", "usia_tahun", "umur_tahun", "usia_pegawai", "umur_pegawai"],
  birthDate: ["tanggal_lahir", "tanggallahir", "tgl_lahir", "tgllahir", "birth_date", "birthdate", "date_of_birth", "dob"],
  education: [
    "education",
    "pendidikan",
    "pendidikan_terakhir",
  ...
  department: ["department", "golongan", "grade", "rank", "golongan_ruang", "golonganruang", "gol_ruang"],
} as const
...
const fallbackDepartmentCategory: CategoryRow[] = fallbackDepartmentData.map((item, index) => ({
  name: item.dept,
...
function pickField(row: RawRow, aliases: Set<string>, override?: string): unknown {
  if (override) {
    const overrideKey = Object.keys(row).find((key) => key.toLowerCase() === override.toLowerCase())
    if (overrideKey) {
...
function matchAlias(value: string, alias: string): boolean {
  ...
  if (alias.length <= 4) {
    return normalized === alias || new RegExp(`\\b${alias}\\b`).test(normalized)
  }
...
function normalizeGender(value: unknown): GenderKey | null {
  ...
  if (raw === "p" || raw === "pr" || raw === "f" || raw === "female" || raw === "wanita" || raw === "perempuan") {
    return "female"
  }
...
function normalizeAgeBucket(value: unknown): (typeof AGE_BUCKETS)[number] | null {
  if (typeof value === "string") {
    const raw = value.trim()
    if (!raw) {
      return null
...
function normalizeEducation(value: unknown): (typeof EDUCATION_LEVELS)[number] | null {
  if (value === null || value === undefined) {
    return null
  }
  const raw = String(value).trim().toLowerCase()
  ...
  if (compact.includes("s2") || raw.includes("magister") || raw.includes("master")) {
    return "S2"
  }
  if (compact.includes("s1") || compact.includes("d4") || /\bd\s*-?\s*iv\b/.test(raw) || raw.includes("sarjana")) {
    return "S1-D4"
  }
  if (compact.includes("d1") || compact.includes("d2") || compact.includes("d3") || /\bd\s*-?\s*i{1,3}\b/.test(raw) || raw.includes("diploma")) {
    return "D1-D3"
  }
  if (raw.includes("slta") || raw.includes("sma") || raw.includes("smk") || raw.includes("ma")) {
    return "SLTA"
  }
  return null
}
...
export function buildStatsFromRows(rows: RawRow[]): EmployeeStats {
  const statusCounts: StatusCounts = { PNS: 0, CPNS: 0, PPPK: 0, PPnPN: 0, KI: 0 }
  ...
  const positionMap = new Map<string, { label: string; male: number; female: number }>()
  const departmentMap = new Map<string, { label: string; male: number; female: number }>()

  let maleEmployees = 0
  let femaleEmployees = 0

  for (const row of rows) {
    const status = normalizeStatus(pickField(row, FIELD_SETS.status, FIELD_OVERRIDES.status))
    if (status) {
      statusCounts[status] += 1
    }

    const gender = normalizeGender(pickField(row, FIELD_SETS.gender, FIELD_OVERRIDES.gender))
    if (gender === "male") {
      maleEmployees += 1
    ...
    const ageBucket =
      normalizeAgeBucket(ageValue) ?? normalizeAgeBucket(pickField(row, FIELD_SETS.birthDate, FIELD_OVERRIDES.birthDate))
    if (ageBucket && gender) {
    ...
    const education = normalizeEducation(pickField(row, FIELD_SETS.education, FIELD_OVERRIDES.education))
    if (education && status) {
    ...
    const positionLabel = normalizeLabel(pickField(row, FIELD_SETS.position, FIELD_OVERRIDES.position))
    if (positionLabel && gender) {
      const key = positionLabel.toLowerCase()
      const existing = positionMap.get(key) ?? { label: positionLabel, male: 0, female: 0 }
      if (gender === "male") {
        existing.male += 1
      } else {
        existing.female += 1
    ...
    const departmentLabel = normalizeLabel(pickField(row, FIELD_SETS.department, FIELD_OVERRIDES.department))
    if (departmentLabel && gender) {
      const key = departmentLabel.toLowerCase()
      const existing = departmentMap.get(key) ?? { label: departmentLabel, male: 0, female: 0 }
      if (gender === "male") {
        existing.male += 1
      } else {
        existing.female += 1
  ...
  const ageCategoryData = STATUS_ORDER.filter((status) => status !== "PPnPN").map((status) => ({
    name: status,
  ...
  const positionData = Array.from(positionMap.values())
    ...
    .map((item) => ({ position: item.label, male: item.male, female: item.female }))
  ...
  const departmentData = Array.from(departmentMap.values())
    .sort((a, b) => a.label.localeCompare(b.label, "id"))
    .map((item) => ({ dept: item.label, male: item.male, female: item.female }))
  ...
  const totalEmployees = rows.length > 0 ? rows.length : STATUS_ORDER.reduce((sum, status) => sum + statusCounts[status], 0)
